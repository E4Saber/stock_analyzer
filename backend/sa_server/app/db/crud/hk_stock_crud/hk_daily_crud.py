...
class HkDailyCRUD:
    """
    CRUD operations for Hong Kong stock daily market data.
    
    Provides methods to create, read, update, and delete Hong Kong stock daily records in the database.
    """

    def __init__(self, db):
        self.db = db

    async def create_hk_daily(self, data: HkDailyData) -> int:
        """
        Create a new Hong Kong stock daily record in the database.
        
        Args:
            data (HkDailyData): The daily data to insert
    ...
    async def get_hk_daily_by_id(self, id: int) -> Optional[HkDailyData]:
        """
        Retrieve a Hong Kong stock daily record by its ID.
        
        Args:
            id (int): The ID of the record to retrieve
            
        Returns:
            HkDailyData | None: The daily data if found, None otherwise
        """
        query = "SELECT * FROM hk_daily WHERE id = $1"
        row = await self.db.fetchrow(query, id)
        return HkDailyData.model_validate(dict(row)) if row else None
    
    async def get_hk_daily_by_key(self, ts_code: str, trade_date: str) -> Optional[HkDailyData]:
        """
        Retrieve a Hong Kong stock daily record by its unique key (ts_code and trade_date).
        ...
        query = "SELECT * FROM hk_daily WHERE ts_code = $1 AND trade_date = $2::date"
        row = await self.db.fetchrow(query, ts_code, formatted_date)
        return HkDailyData.model_validate(dict(row)) if row else None
    
    async def list_hk_daily_by_ts_code(self, 
                                     ts_code: str, 
                                     start_date: Optional[str] = None, 
                                     end_date: Optional[str] = None,
                                     limit: int = 100,
                                     offset: int = 0) -> List[HkDailyData]:
        """
        List Hong Kong stock daily records for a specific stock code with optional date range.
        ...
        Returns:
            list[HkDailyData]: List of Hong Kong stock daily data
        """
        params = [ts_code]
        query_parts = ["SELECT * FROM hk_daily WHERE ts_code = $1"]
        ...
        return [HkDailyData.model_validate(dict(row)) for row in rows]
    
    async def list_hk_daily_by_trade_date(self, 
                                        trade_date: str,
                                        limit: int = 100,
                                        offset: int = 0) -> List[HkDailyData]:
        """
        List Hong Kong stock daily records for a specific trade date.
    ...
    async def update_hk_daily(self, id: int, updates: Dict[str, Any]) -> bool:
        """
        Update a Hong Kong stock daily record by its ID.
    ...
    async def update_hk_daily_by_key(self, ts_code: str, trade_date: str, updates: Dict[str, Any]) -> bool:
        """
        Update a Hong Kong stock daily record by its unique key (ts_code and trade_date).
    ...
    async def delete_hk_daily(self, id: int) -> bool:
        """
        Delete a Hong Kong stock daily record by its ID.
        ...
        """
        query = "DELETE FROM hk_daily WHERE id = $1"
        result = await self.db.execute(query, id)
        
        # 检查是否有行被删除
        return 'DELETE 1' in result
    
    async def delete_hk_daily_by_key(self, ts_code: str, trade_date: str) -> bool:
        """
        Delete a Hong Kong stock daily record by its unique key (ts_code and trade_date).
        ...
        query = "DELETE FROM hk_daily WHERE ts_code = $1 AND trade_date = $2::date"
        result = await self.db.execute(query, ts_code, formatted_date)
        
        # 检查是否有行被删除
        return 'DELETE 1' in result
    
    async def delete_hk_daily_by_ts_code(self, ts_code: str) -> int:
        """
        Delete all Hong Kong stock daily records for a specific stock code.
        ...
        """
        query = "DELETE FROM hk_daily WHERE ts_code = $1"
    ...
    async def delete_hk_daily_by_trade_date(self, trade_date: str) -> int:
        """
        Delete all Hong Kong stock daily records for a specific trade date.
        ...
        query = "DELETE FROM hk_daily WHERE trade_date = $1::date"
    ...
    async def get_hk_daily(self, 
        ...
        """
        # 初始化查询部分
        query_parts = ["SELECT * FROM hk_daily"]
        params = []
        param_idx = 1
        ...
        if filters and len(filters) > 0:
            ...
            for key, value in filters.items():
                ...
                # 验证字段是否有效（避免SQL注入）
                valid_fields = [f.name for f in HkDailyData.model_fields.values()]
