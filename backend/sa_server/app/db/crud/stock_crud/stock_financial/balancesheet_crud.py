...
class BalancesheetCRUD:
    """
    CRUD operations for balance sheet data.
    
    Provides methods to create, read, update, and delete balance sheet records in the database.
    """

    def __init__(self, db):
        self.db = db

    async def create_balancesheet(self, data: BalancesheetData) -> int:
        """
        Create a new balance sheet record in the database.
        
        Args:
            data (BalancesheetData): The balance sheet data to insert
    ...
    async def get_balancesheet_by_id(self, id: int) -> Optional[BalancesheetData]:
        """
        Retrieve a balance sheet record by its ID.
        
        Args:
            id (int): The ID of the record to retrieve
            
        Returns:
            BalancesheetData | None: The balance sheet data if found, None otherwise
        """
        query = "SELECT * FROM balancesheet WHERE id = $1"
        row = await self.db.fetchrow(query, id)
        return BalancesheetData.model_validate(dict(row)) if row else None
    
    async def get_balancesheet_by_ts_code_and_end_date(self, ts_code: str, end_date: str) -> Optional[BalancesheetData]:
        """
        Retrieve a balance sheet record by its TS code and report date.
        ...
        query = "SELECT * FROM balancesheet WHERE ts_code = $1 AND end_date = $2::date"
        row = await self.db.fetchrow(query, ts_code, formatted_end_date)
        return BalancesheetData.model_validate(dict(row)) if row else None
    
    async def list_balancesheet_by_ts_code(self, ts_code: str, limit: int = 10) -> List[BalancesheetData]:
        """
        List balance sheet records for a specific stock.
        ...
        Returns:
            list[BalancesheetData]: List of balance sheet data
        """
        query = """
        ...
        return [BalancesheetData.model_validate(dict(row)) for row in rows]
    
    async def update_balancesheet(self, id: int, updates: Dict[str, Any]) -> bool:
        """
        Update a balance sheet record by its ID.
    ...
    async def update_balancesheet_by_ts_code_and_date(self, ts_code: str, end_date: str, updates: Dict[str, Any]) -> bool:
        """
        Update a balance sheet record by TS code and end date.
    ...
    async def delete_balancesheet(self, id: int) -> bool:
        """
        Delete a balance sheet record by its ID.
        ...
        """
        query = "DELETE FROM balancesheet WHERE id = $1"
        result = await self.db.execute(query, id)
        
        # 检查是否有行被删除
        return 'DELETE 1' in result
    
    async def delete_balancesheet_by_ts_code(self, ts_code: str) -> int:
        """
        Delete all balance sheet records for a specific stock.
        ...
        """
        query = "DELETE FROM balancesheet WHERE ts_code = $1"
    ...
    async def get_latest_reports(self, limit: int = 100) -> List[BalancesheetData]:
        """
        Get the latest balance sheet reports across all stocks.
    ...
    async def get_reports_by_period(self, period_type: str, year: int, quarter: Optional[int] = None) -> List[BalancesheetData]:
        """
        Get balance sheet reports for a specific reporting period.
    ...
    async def get_balancesheet(self, 
                         filters: Optional[Dict[str, Any]] = None, 
                         order_by: Optional[List[str]] = None,
                         limit: Optional[int] = None,
                         offset: Optional[int] = None) -> List[BalancesheetData]:
        """
        ...
        """
        # 初始化查询部分
        query_parts = ["SELECT * FROM balancesheet"]
        params = []
        param_idx = 1
        ...
        if filters and len(filters) > 0:
            ...
            for key, value in filters.items():
                ...
                # 验证字段是否有效（避免SQL注入）
                valid_fields = [f.name for f in BalancesheetData.model_fields.values()]
