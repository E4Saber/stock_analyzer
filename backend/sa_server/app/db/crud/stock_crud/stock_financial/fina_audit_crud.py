...
class FinaAuditCRUD:
    """
    CRUD operations for financial audit data.
    
    Provides methods to create, read, update, and delete financial audit records in the database.
    """

    def __init__(self, db):
        self.db = db

    async def create_fina_audit(self, data: FinaAuditData) -> int:
        """
        Create a new financial audit record in the database.
        
        Args:
            data (FinaAuditData): The financial audit data to insert
    ...
    async def get_fina_audit_by_id(self, id: int) -> Optional[FinaAuditData]:
        """
        Retrieve a financial audit record by its ID.
        
        Args:
            id (int): The ID of the record to retrieve
            
        Returns:
            FinaAuditData | None: The financial audit data if found, None otherwise
        """
        query = "SELECT * FROM fina_audit WHERE id = $1"
        row = await self.db.fetchrow(query, id)
        return FinaAuditData.model_validate(dict(row)) if row else None
    
    async def get_fina_audit_by_ts_code_and_end_date(self, ts_code: str, end_date: str) -> Optional[FinaAuditData]:
        """
        Retrieve a financial audit record by its TS code and end date.
        ...
        query = "SELECT * FROM fina_audit WHERE ts_code = $1 AND end_date = $2::date"
        row = await self.db.fetchrow(query, ts_code, formatted_end_date)
        return FinaAuditData.model_validate(dict(row)) if row else None
    
    async def list_fina_audit_by_ts_code(self, ts_code: str, limit: int = 10) -> List[FinaAuditData]:
        """
        List financial audit records for a specific stock.
        ...
        Returns:
            list[FinaAuditData]: List of financial audit data
        """
        query = """
            SELECT * FROM fina_audit 
            WHERE ts_code = $1 
            ORDER BY end_date DESC 
            LIMIT $2
        """
        rows = await self.db.fetch(query, ts_code, limit)
        return [FinaAuditData.model_validate(dict(row)) for row in rows]
    
    async def list_fina_audit_by_audit_agency(self, audit_agency: str, limit: int = 100) -> List[FinaAuditData]:
        """
        List financial audit records for a specific audit agency.
        
        Args:
            audit_agency (str): The name of the audit agency
            limit (int): Maximum number of records to retrieve
        ...
        query = """
            SELECT * FROM fina_audit 
            WHERE audit_agency = $1 
            ORDER BY end_date DESC, ts_code
            LIMIT $2
        """
        rows = await self.db.fetch(query, audit_agency, limit)
        return [FinaAuditData.model_validate(dict(row)) for row in rows]
    
    async def update_fina_audit(self, id: int, updates: Dict[str, Any]) -> bool:
        """
        Update a financial audit record by its ID.
    ...
    async def update_fina_audit_by_ts_code_and_date(self, ts_code: str, end_date: str, updates: Dict[str, Any]) -> bool:
        """
        Update a financial audit record by TS code and end date.
    ...
    async def delete_fina_audit(self, id: int) -> bool:
        """
        Delete a financial audit record by its ID.
        ...
        """
        query = "DELETE FROM fina_audit WHERE id = $1"
        result = await self.db.execute(query, id)
        
        # 检查是否有行被删除
        return 'DELETE 1' in result
    
    async def delete_fina_audit_by_ts_code(self, ts_code: str) -> int:
        """
        Delete all financial audit records for a specific stock.
        ...
        """
        query = "DELETE FROM fina_audit WHERE ts_code = $1"
    ...
    async def get_latest_reports(self, limit: int = 100) -> List[FinaAuditData]:
        """
        Get the latest financial audit reports across all stocks.
    ...
    async def get_fina_audit(self, 
                          filters: Optional[Dict[str, Any]] = None, 
                          order_by: Optional[List[str]] = None,
                          limit: Optional[int] = None,
                          offset: Optional[int] = None) -> List[FinaAuditData]:
        """
        ...
        """
        # 初始化查询部分
        query_parts = ["SELECT * FROM fina_audit"]
        params = []
        param_idx = 1
        ...
        if filters and len(filters) > 0:
            ...
            for key, value in filters.items():
                ...
                # 验证字段是否有效（避免SQL注入）
                valid_fields = [f.name for f in FinaAuditData.model_fields.values()]
