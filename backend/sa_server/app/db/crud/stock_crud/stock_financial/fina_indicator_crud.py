...
class FinaIndicatorCRUD:
    """
    CRUD operations for financial indicator data.
    
    Provides methods to create, read, update, and delete financial indicator records in the database.
    """

    def __init__(self, db):
        self.db = db

    async def create_fina_indicator(self, data: FinaIndicatorData) -> int:
        """
        Create a new financial indicator record in the database.
        
        Args:
            data (FinaIndicatorData): The financial indicator data to insert
    ...
    async def get_fina_indicator_by_id(self, id: int) -> Optional[FinaIndicatorData]:
        """
        Retrieve a financial indicator record by its ID.
        
        Args:
            id (int): The ID of the record to retrieve
            
        Returns:
            FinaIndicatorData | None: The financial indicator data if found, None otherwise
        """
        query = "SELECT * FROM fina_indicator WHERE id = $1"
        row = await self.db.fetchrow(query, id)
        return FinaIndicatorData.model_validate(dict(row)) if row else None
    
    async def get_fina_indicator_by_ts_code_and_end_date(self, ts_code: str, end_date: str) -> Optional[FinaIndicatorData]:
        """
        Retrieve a financial indicator record by its TS code and end date.
        ...
        query = "SELECT * FROM fina_indicator WHERE ts_code = $1 AND end_date = $2::date"
        row = await self.db.fetchrow(query, ts_code, formatted_end_date)
        return FinaIndicatorData.model_validate(dict(row)) if row else None
    
    async def list_fina_indicator_by_ts_code(self, ts_code: str, limit: int = 10) -> List[FinaIndicatorData]:
        """
        List financial indicator records for a specific stock.
        ...
        Returns:
            list[FinaIndicatorData]: List of financial indicator data
        """
        query = """
        ...
        return [FinaIndicatorData.model_validate(dict(row)) for row in rows]
    
    async def update_fina_indicator(self, id: int, updates: Dict[str, Any]) -> bool:
        """
        Update a financial indicator record by its ID.
    ...
    async def update_fina_indicator_by_ts_code_and_date(self, ts_code: str, end_date: str, updates: Dict[str, Any]) -> bool:
        """
        Update a financial indicator record by TS code and end date.
    ...
    async def delete_fina_indicator(self, id: int) -> bool:
        """
        Delete a financial indicator record by its ID.
        ...
        """
        query = "DELETE FROM fina_indicator WHERE id = $1"
        result = await self.db.execute(query, id)
        
        # 检查是否有行被删除
        return 'DELETE 1' in result
    
    async def delete_fina_indicator_by_ts_code(self, ts_code: str) -> int:
        """
        Delete all financial indicator records for a specific stock.
        ...
        """
        query = "DELETE FROM fina_indicator WHERE ts_code = $1"
    ...
    async def get_latest_reports(self, limit: int = 100) -> List[FinaIndicatorData]:
        """
        Get the latest financial indicator reports across all stocks.
    ...
    async def get_reports_by_period(self, period_type: str, year: int, quarter: Optional[int] = None) -> List[FinaIndicatorData]:
        """
        Get financial indicator reports for a specific reporting period.
    ...
    async def get_fina_indicator(self, 
                             filters: Optional[Dict[str, Any]] = None, 
                             order_by: Optional[List[str]] = None,
                             limit: Optional[int] = None,
                             offset: Optional[int] = None) -> List[FinaIndicatorData]:
        """
        ...
        """
        # 初始化查询部分
        query_parts = ["SELECT * FROM fina_indicator"]
        params = []
        param_idx = 1
        ...
        if filters and len(filters) > 0:
            ...
            for key, value in filters.items():
                ...
                # 验证字段是否有效（避免SQL注入）
                valid_fields = [f.name for f in FinaIndicatorData.model_fields.values()]
