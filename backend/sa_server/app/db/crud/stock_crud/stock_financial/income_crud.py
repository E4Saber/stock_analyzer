...
class IncomeCRUD:
    """
    CRUD operations for income statement data.
    
    Provides methods to create, read, update, and delete income statement records in the database.
    """

    def __init__(self, db):
        self.db = db

    async def create_income(self, data: IncomeData) -> int:
        """
        Create a new income statement record in the database.
        
        Args:
            data (IncomeData): The income statement data to insert
    ...
    async def get_income_by_id(self, id: int) -> Optional[IncomeData]:
        """
        Retrieve an income statement record by its ID.
        
        Args:
            id (int): The ID of the record to retrieve
            
        Returns:
            IncomeData | None: The income statement data if found, None otherwise
        """
        query = "SELECT * FROM income WHERE id = $1"
        row = await self.db.fetchrow(query, id)
        return IncomeData.model_validate(dict(row)) if row else None
    
    async def get_income_by_ts_code_and_end_date(self, ts_code: str, end_date: str) -> Optional[IncomeData]:
        """
        Retrieve an income statement record by its TS code and report date.
        ...
        query = "SELECT * FROM income WHERE ts_code = $1 AND end_date = $2::date"
        row = await self.db.fetchrow(query, ts_code, formatted_end_date)
        return IncomeData.model_validate(dict(row)) if row else None
    
    async def list_income_by_ts_code(self, ts_code: str, limit: int = 10) -> List[IncomeData]:
        """
        List income statement records for a specific stock.
        ...
        Returns:
            list[IncomeData]: List of income statement data
        """
        query = """
            SELECT * FROM income 
            WHERE ts_code = $1 
        ...
        return [IncomeData.model_validate(dict(row)) for row in rows]
    
    async def update_income(self, id: int, updates: Dict[str, Any]) -> bool:
        """
        Update an income statement record by its ID.
    ...
    async def update_income_by_ts_code_and_date(self, ts_code: str, end_date: str, updates: Dict[str, Any]) -> bool:
        """
        Update an income statement record by TS code and end date.
    ...
    async def delete_income(self, id: int) -> bool:
        """
        Delete an income statement record by its ID.
    ...
    async def delete_income_by_ts_code(self, ts_code: str) -> int:
        """
        Delete all income statement records for a specific stock.
        ...
        """
        query = "DELETE FROM income WHERE ts_code = $1"
    ...
    async def get_latest_reports(self, limit: int = 100) -> List[IncomeData]:
        """
        Get the latest income statement reports across all stocks.
    ...
    async def get_reports_by_period(self, period_type: str, year: int, quarter: Optional[int] = None) -> List[IncomeData]:
        """
        Get income statement reports for a specific reporting period.
    ...
    """
        ...
        income_records = await crud.get_income(
            filters={'ts_code': '600000.SH', 'report_type': '1'}
        )
        ...
        income_records = await crud.get_income(
            filters={
                'ts_code__like': '600%',
                'end_date__gt': '20230101',
                'n_income__ge': 1000000
            }
        )

        # With custom sorting
        income_records = await crud.get_income(
            filters={'ts_code__in': ['600000.SH', '601398.SH']},
    ...
    async def get_income(self, 
                         ...
                         offset: Optional[int] = None) -> List[IncomeData]:
        """
        ...
        """
        # 初始化查询部分
        query_parts = ["SELECT * FROM income"]
        params = []
        param_idx = 1
        ...
        if filters and len(filters) > 0:
            ...
            for key, value in filters.items():
                ...
                # 验证字段是否有效（避免SQL注入）
                # 这里假设IncomeData有一个__fields__属性包含所有有效字段，实际使用时可能需要调整
                valid_fields = [f.name for f in IncomeData.model_fields.values()]
