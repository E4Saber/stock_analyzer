...
class StkHoldernumberCRUD:
    """
    CRUD operations for stock holder number data.
    
    Provides methods to create, read, update, and delete stock holder number records in the database.
    """

    def __init__(self, db):
        self.db = db

    async def create_stk_holdernumber(self, data: StkHoldernumberData) -> int:
        """
        Create a new stock holder number record in the database.
        
        Args:
            data (StkHoldernumberData): The holder number data to insert
    ...
    async def get_stk_holdernumber_by_id(self, id: int) -> Optional[StkHoldernumberData]:
        """
        Retrieve a stock holder number record by its ID.
        
        Args:
            id (int): The ID of the record to retrieve
            
        Returns:
            StkHoldernumberData | None: The holder number data if found, None otherwise
        """
        query = "SELECT * FROM stk_holdernumber WHERE id = $1"
        row = await self.db.fetchrow(query, id)
        return StkHoldernumberData.model_validate(dict(row)) if row else None
    
    async def get_stk_holdernumber_by_ts_code_and_date(self, ts_code: str, end_date: str) -> Optional[StkHoldernumberData]:
        """
        Retrieve stock holder number record by its TS code and end date.
        ...
        query = "SELECT * FROM stk_holdernumber WHERE ts_code = $1 AND end_date = $2::date"
        row = await self.db.fetchrow(query, ts_code, formatted_end_date)
        return StkHoldernumberData.model_validate(dict(row)) if row else None
    
    async def list_stk_holdernumber_by_ts_code(self, ts_code: str, limit: int = 100) -> List[StkHoldernumberData]:
        """
        List stock holder number records for a specific stock.
        ...
        Returns:
            list[StkHoldernumberData]: List of holder number data
        """
        query = """
        ...
        return [StkHoldernumberData.model_validate(dict(row)) for row in rows]
    
    async def update_stk_holdernumber(self, id: int, updates: Dict[str, Any]) -> bool:
        """
        Update a stock holder number record by its ID.
    ...
    async def update_stk_holdernumber_by_key(self, ts_code: str, end_date: str, updates: Dict[str, Any]) -> bool:
        """
        Update a stock holder number record by its unique key (ts_code, end_date).
    ...
    async def delete_stk_holdernumber(self, id: int) -> bool:
        """
        Delete a stock holder number record by its ID.
        ...
        """
        query = "DELETE FROM stk_holdernumber WHERE id = $1"
        result = await self.db.execute(query, id)
        
        # 检查是否有行被删除
        return 'DELETE 1' in result
    
    async def delete_stk_holdernumber_by_ts_code(self, ts_code: str) -> int:
        """
        Delete all stock holder number records for a specific stock.
        ...
        """
        query = "DELETE FROM stk_holdernumber WHERE ts_code = $1"
    ...
    async def delete_stk_holdernumber_by_period(self, ts_code: str, end_date: str) -> int:
        """
        Delete all stock holder number records for a specific stock and report period.
        ...
        query = "DELETE FROM stk_holdernumber WHERE ts_code = $1 AND end_date = $2::date"
    ...
    async def get_stk_holdernumber(self, 
                                 filters: Optional[Dict[str, Any]] = None, 
                                 order_by: Optional[List[str]] = None,
                                 limit: Optional[int] = None,
                                 offset: Optional[int] = None) -> List[StkHoldernumberData]:
        """
        ...
        参数:
            ...
            order_by: 排序字段列表，可以在字段前加"-"表示降序，例如['-end_date']
            limit: 最大返回记录数
        ...
        """
        # 初始化查询部分
        query_parts = ["SELECT * FROM stk_holdernumber"]
        params = []
        param_idx = 1
        ...
        if filters and len(filters) > 0:
            ...
            for key, value in filters.items():
                ...
                # 验证字段是否有效（避免SQL注入）
                valid_fields = [f.name for f in StkHoldernumberData.model_fields.values()]
