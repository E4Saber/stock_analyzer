...
from app.external.tushare_api.stock.reference_data_api import get_top10_holders
from app.data.db_modules.stock_modules.reference_data.top10_holders import Top10HoldersData

class Top10HoldersService:
    """十大股东数据导入服务，使用PostgreSQL COPY命令高效导入数据"""
    ...
    async def import_top10_holders_data(self, ts_code: Optional[str] = None, 
        ...
        """
        # 从Tushare获取数据
        df_result = get_top10_holders(ts_code=ts_code, period=period, ann_date=ann_date,
                                   start_date=start_date, end_date=end_date)
        
        if df_result is None or df_result.empty:
            print(f"未找到十大股东数据: ts_code={ts_code}, period={period}")
            return 0
            ...
            try:
                ...
                for record in batch:
                    try:
                        ...
                        holder_data = Top10HoldersData(**record)
                        holder_data_list.append(holder_data)
                    except Exception as e:
                        print(f"创建Top10HoldersData对象失败 {record.get('ts_code', '未知')}, {record.get('end_date', '未知')}, {record.get('holder_name', '未知')}: {str(e)}")
                
                # 使用COPY命令批量导入
                if holder_data_list:
                    inserted = await self.batch_upsert_top10_holders(holder_data_list)
                    total_count += inserted
                    print(f"批次导入成功: {inserted} 条十大股东记录")
            except Exception as e:
                print(f"批次导入失败: {str(e)}")
        
        return total_count
    
    async def batch_upsert_top10_holders(self, holders_list: List[Top10HoldersData]) -> int:
        """
        使用PostgreSQL的COPY命令进行高效批量插入或更新
        
        参数:
            holders_list: 要插入或更新的十大股东数据列表
        ...
        async with self.db.pool.acquire() as conn:
            try:
                ...
                async with conn.transaction():
                    ...
                    # 首先获取原表的列信息
                    column_types = await self._get_column_type(conn, 'top10_holders', columns)
                    ...
                    await conn.execute(f'''
                        CREATE TEMP TABLE temp_top10_holders (
                            {', '.join(column_defs)}
                        ) ON COMMIT DROP
                    ''')
                    
                    # 使用COPY命令将数据复制到临时表
                    await conn.copy_records_to_table('temp_top10_holders', records=records, columns=columns)
                    ...
                    result = await conn.execute(f'''
                        INSERT INTO top10_holders ({', '.join(columns)})
                        SELECT {', '.join(columns)} FROM temp_top10_holders
...
# 快捷函数，用于导入特定股票的十大股东数据
async def import_stock_top10_holders(db, ts_code: str, batch_size: int = 1000):
    """
    导入特定股票的十大股东数据
    ...
    返回:
        导入的记录数
    """
    service = Top10HoldersService(db)
    count = await service.import_top10_holders_data(ts_code=ts_code, batch_size=batch_size)
    print(f"成功导入 {count} 条股票 {ts_code} 的十大股东记录")
    return count


# 快捷函数，用于导入特定公告日期的十大股东数据
async def import_ann_date_top10_holders(db, ann_date: str, batch_size: int = 1000):
    """
    导入特定公告日期的十大股东数据
    ...
    """
    service = Top10HoldersService(db)
    count = await service.import_top10_holders_data(ann_date=ann_date, batch_size=batch_size)
    print(f"成功导入 {count} 条公告日期为 {ann_date} 的十大股东记录")
    return count


# 快捷函数，用于导入特定报告期的十大股东数据
async def import_period_top10_holders(db, period: str, batch_size: int = 1000):
    """
    导入特定报告期的十大股东数据
    ...
    """
    service = Top10HoldersService(db)
    count = await service.import_top10_holders_data(period=period, batch_size=batch_size)
    ...
    print(f"成功导入 {count} 条报告期为 {period_info} 的十大股东记录")
    return count


# 快捷函数，用于导入特定日期范围的十大股东数据
async def import_date_range_top10_holders(db, start_date: str, end_date: str, batch_size: int = 1000):
    """
    导入特定日期范围的十大股东数据
    ...
    """
    service = Top10HoldersService(db)
    count = await service.import_top10_holders_data(start_date=start_date, end_date=end_date, batch_size=batch_size)
    print(f"成功导入 {count} 条日期范围为 {start_date} 至 {end_date} 的十大股东记录")
    return count


# 综合导入函数，支持多种参数组合
async def import_top10_holders_with_params(db, **kwargs):
    """
    根据提供的参数导入十大股东数据
    ...
    # 导入数据
    count = await service.import_top10_holders_data(batch_size=batch_size, **kwargs)
    print(f"成功导入 {count} 条十大股东记录 ({params_info})")
    return count


# 导入所有十大股东数据
async def import_all_top10_holders(db, batch_size: int = 1000):
    """
    导入所有可获取的十大股东数据
    ...
    print("开始导入所有十大股东数据，此操作可能需要较长时间...")
    count = await service.import_top10_holders_data(batch_size=batch_size)
    
    print(f"成功导入所有十大股东数据，共 {count} 条记录")
    return count


# 动态查询十大股东数据
async def query_top10_holders_data(db, 
    ...
    示例:
        # 查询某股票最近一期的十大股东，按持股比例降序排列
        data = await query_top10_holders_data(
            db,
    ...
    """
    from app.db.crud.stock_crud.reference_data.top10_holders_crud import Top10HoldersCRUD
    
    crud = Top10HoldersCRUD(db)
    results = await crud.get_top10_holders(
        filters=filters,
        order_by=order_by,
        limit=limit,
        offset=offset
...
# 查询特定公司最近几期的十大股东变化
async def get_top10_holders_history(db, ts_code: str, periods: int = 4):
    """
    查询特定公司最近几期的十大股东变化历史
    ...
    返回:
        List[Top10HoldersData]: 按报告期降序排列的十大股东数据
    """
    from app.db.crud.stock_crud.reference_data.top10_holders_crud import Top10HoldersCRUD
...
async def track_shareholder_holdings(db, holder_name: str, end_date: Optional[str] = None, limit: int = 100):
    ...
    返回:
        List[Top10HoldersData]: 该股东在不同公司的持股数据
    """
    ...
    return await query_top10_holders_data(
        db,
        filters=filters,
        order_by=['-hold_ratio', 'ts_code'],
        limit=limit
...
async def analyze_holder_types(db, ts_code: str, end_date: Optional[str] = None):
    ...
    # 获取该报告期的十大股东数据
    holders_data = await query_top10_holders_data(
        db,
        filters=filters,
        order_by=['-hold_ratio']
    )
...
async def detect_holder_changes(db, ts_code: str, current_period: str, compare_period: str):
    ...
    # 获取两个报告期的十大股东数据
    current_holders = await crud.get_top10_holders_by_ts_code_and_date(ts_code, current_period)
    compare_holders = await crud.get_top10_holders_by_ts_code_and_date(ts_code, compare_period)
